/**
 * <slikk-credit> — maximum-isolation attribution badge (Shadow DOM).
 *
 * Use this variant ONLY when the host site has hostile/aggressive global CSS
 * (heavy resets, `* { … !important }`, Tailwind preflight on everything, etc.)
 * and you want a hard guarantee that nothing leaks in or out. Shadow DOM makes
 * style interference physically impossible in both directions.
 *
 * Trade-off vs. the plain HTML/React snippet: this needs ~1 small inline script
 * (a `script-src` under strict CSP — no remote load, no eval) and won't render
 * if JavaScript is disabled. For most sites the plain snippet is enough.
 *
 * Usage:
 ...
 *
 * Optional attributes:
 *   text  — line before the wordmark (default: "Lovingly over‑engineered by")
 *   href  — link target (default: https://slikk.dev/)
 */
(function () {
  if (customElements.get('slikk-credit')) return;
  ...
  class SlikkCredit extends HTMLElement {
    connectedCallback() {
      const text = this.getAttribute('text') || 'Lovingly over‑engineered by';
      const href = this.getAttribute('href') || 'https://slikk.dev/';
      const root = this.attachShadow({ mode: 'open' });
      root.innerHTML = `
        <style>
          :host{display:inline-block}
          a{box-sizing:border-box;display:inline-flex;align-items:center;gap:.5em;
            font:600 13px/1 system-ui,-apple-system,"Segoe UI",Roboto,sans-serif;color:#6b7280;
            text-decoration:none;padding:6px 2px;transition:color .18s ease}
          a:hover,a:focus-visible{color:#111827}
          a:focus-visible{outline:2px solid #FF2D6B;outline-offset:3px;border-radius:4px}
          .t{opacity:.9}
          svg{height:15px;width:auto;display:block}
          @media (prefers-reduced-motion:reduce){a{transition:none}}
        </style>
        <a href="${href}" target="_blank" rel="nofollow noopener noreferrer"
           aria-label="Developed by Slikk.Dev — opens slikk.dev in a new tab">
          <span class="t">${text}</span>
          <svg viewBox="25 -738 3844 756" aria-hidden="true" focusable="false">
  ...
  customElements.define('slikk-credit', SlikkCredit);
})();
