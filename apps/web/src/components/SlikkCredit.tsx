/**
 * SlikkCredit — the Slikk.Dev attribution badge, ported from the agency's
 * attribution kit (vendored read-only at .references/attribution-kit/).
 *
 * Self-contained and dependency-free: the wordmark is outlined vector paths
 * (no font request), styles are scoped `dsk-` classes isolated behind
 * `all:revert`, and it ships no JS beyond its own markup.
 *
 * Kit invariants (do not change): the SVG paths + viewBox, the #FF2D6B accent
 * dot and focus ring, `rel="nofollow noopener noreferrer"`, and the
 * text-then-wordmark structure. Only lettering color/typography, mark size,
 * and placement adapt to the host site — here the lettering rides the theme's
 * neutral tokens (RGB triplets from src/index.css, dark values as fallbacks):
 * neutral-400 at rest — the AA-checked muted-body step in both themes —
 * brightening to neutral-300 on hover/focus like the footer's other links.
 */
type SlikkCreditProps = {
  /** The maker's-mark line before the wordmark (reads into "Slikk.Dev"). */
  text?: string;
  href?: string;
  /** Positioning/spacing hook for the host footer only. */
  className?: string;
};
...
.dsk-credit{all:revert;box-sizing:border-box;display:inline-flex;align-items:center;gap:.5em;
  font-family:"Nunito Variable",Nunito,ui-rounded,system-ui,-apple-system,"Segoe UI",Roboto,sans-serif;
  font-weight:600;font-size:12px;line-height:1;
  color:rgb(var(--t-neutral-400,164 156 200));text-decoration:none;padding:6px 2px;border:0;
  background:none;transition:color .18s ease}
.dsk-credit:hover,.dsk-credit:focus-visible{color:rgb(var(--t-neutral-300,200 194 224))}
.dsk-credit:focus-visible{outline:2px solid #FF2D6B;outline-offset:3px;border-radius:4px}
.dsk-credit__text{opacity:.9}
.dsk-credit__mark{height:14px;width:auto;display:block}
@media (prefers-reduced-motion:reduce){.dsk-credit{transition:none}}`,
        }}
      />
      <a
        className={`dsk-credit ${className}`.trim()}
        href={href}
        target="_blank"
      ...
      >
        ...
        <svg className="dsk-credit__mark" viewBox="25 -738 3844 756" aria-hidden="true" focusable="false">
          <path
            fill="currentColor"
